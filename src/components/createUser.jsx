...
export default function CreateUser({ setShowPopUp, onCreated }) {
  const [nombreCompleto, setNombreCompleto] = useState('');
  const [correo, setCorreo] = useState('');
  const [password1, setPassword1] = useState('');
  const [password2, setPassword2] = useState('');
  const [roles, setRoles] = useState([]);
  const [rolId, setRolId] = useState(''); // Admin o Técnico (UUID)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (refresh) {
        ...
        if (data.refresh) {
          // IMPORTANTE si usas rotación: sustituir el refresh almacenado
      ...
      await axios.post('http://127.0.0.1:8000/api/usuarios/', body, {
          ...
          <div className='flex flex-col justify-center '>
            <div className='flex items-center justify-center space-x-5'>
              <div className='flex items-center flex-col justify-center pl-2 font-semibold text-xl self-start text-gray-700'>
                <h2 className='leading-relaxed'>Crear un nuevo usuario</h2>
                  ...
                  >
                    ...
                    {roles.map((r) => (
                      <option
                        ...
                        value={r.id}
                      >
                        {r.nombre} {/* mostrará “Administrador” o “Técnico” */}
                      </option>
                    ))}
                  </select>
                </div>
